...
- (void)viewDidLoad
{
    ...
    [img release];
    UIImage *_titleImg = [UIImage imageNamed:@"makeup.png"];
...
-(void)operatStringFirstTime:(NSString*)str
{
    ...
    for (int i = 0; i<[content_Array count]; i++) {
        if ([[[content_Array objectAtIndex:i] objectForKey:@"species"] isEqualToString:@"特效化妆"]) {
...
-(void)operatString:(NSString*)str
{
    ...
    for (int i = 0; i<[update count]; i++) {
        for (int j = 0;j<[content_Array count]; j++) {
            if ([[NSString stringWithFormat:@"%@",[[update objectAtIndex:i] objectForKey:@"is"]]isEqualToString:[NSString stringWithFormat:@"%@",[[content_Array objectAtIndex:j] objectForKey:@"id"]] ] ){
                [content_Array removeObjectAtIndex:j];
...
//    NSLog(@"aaaaaaaaaaaaaaaa%@",content_Array);
    for (int i = 0; i<[content_Array count]; i++) {
       ...
       NSLog(@"aaaaaaaaaaaaaaaa%@",_speciesStr);
        if ([_speciesStr isEqualToString:@"特效化妆"]) {
            [m_requireArray addObject:[content_Array objectAtIndex:i]];
        }
    }
